...
export default function App() {
  const [kpis, setKpis] = useState(() => initialKpis.map(k => ({
    ...k,
  ...
  function handleLog(kpiId, value) {
    setKpis((prev) => prev.map((k) => {
      if (k.id !== kpiId) return k;
      const nextIndex = k.history.length + 1;
      return { ...k, history: [...k.history, { d: `W${nextIndex}`, v: value }] };
    }));
  }

  function handleAddMember(teamId, member) {
    setTeams((prev) => prev.map((t) => t.id === teamId ? { ...t, members: [...t.members, member] } : t));
  }

  function handleAddVertical(newVertical) {
    setTeams((prev) => [...prev, newVertical]);
  }

  function handleAddKpi(newKpi) {
    setKpis((prev) => [...prev, {
      ...newKpi,
      targetType: newKpi.targetType || "monthly",
      targetsList: newKpi.targetsList || [
        { id: "1", label: "CY Target", targetValue: newKpi.target, targetDate: "2026-08-31" }
      ]
    }]);
  }

  function handleEditKpi(updatedKpi) {
  ...
  function handleUpdateProjectStage(projectId, stageIdx, status) {
    setProjects((prev) => prev.map((proj) => {
      if (proj.id !== projectId) return proj;
      const updatedStages = proj.stages.map((stage, idx) => {
